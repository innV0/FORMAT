...
│       ├── markers
│       └── matrices
│           └── params (Possible widget options, e.g., RACI or scale parameters)
└── Markdown Body
    ├── Document Notice (GFM [!NOTE] admonition)
    ├── Concept Blocks (type: text, list, hierarchy)
    │   └── elements (with custom fields in YAML code blocks)
...
The first content in the Markdown body — immediately after the frontmatter delimiter and before any concept block — MUST be a GFM blockquote notice identifying the file as a FORMAT document and pointing to the visual editor.
...
4. Markdown body begins with the required Document Notice (§3.1).
